...
static int wayland_gbm_get_drm_fd(char const *sysname)
{
    const char *seat;
    ...
    udev_list_entry_foreach(entry, udev_enumerate_get_list_entry(e))
    {
        ...
        TRACE("Trying to open drm device %s => fd=%d\n", devnode, drm_fd);
        udev_device_unref(device);
        if (drm_fd >= 0) break;
    }
...
static void wayland_gbm_init_once(void)
{
    ...
    {
        TRACE("Trying to open drm device specified in options: %s\n",
              option_drm_device);
    ...
    if (drm_fd < 0)
    {
        WARN("Failed to open drm device from options, trying to find suitable render node\n");
        drm_fd = wayland_gbm_get_drm_fd(render_node_sysname);
    }

    if (drm_fd < 0)
    {
        WARN("Failed to find a suitable render node, trying default render node\n");
        drm_fd = open(default_render_node, O_RDWR);
    }

    if (drm_fd < 0)
    {
        WARN("Failed to use default render node, trying to find a primary node\n");
        drm_fd = wayland_gbm_get_drm_fd(primary_node_sysname);
    }

    if (drm_fd < 0)
    {
        WARN("Failed to find a suitable primary node, trying default primary node\n");
