...
static void wayland_gl_drawable_update(struct wayland_gl_drawable *gl)
{
    ...
    dmabuf_format = wayland_dmabuf_find_format(dmabuf, drm_format);
    if (dmabuf_format && wayland_dmabuf_format_supports_modifiers(dmabuf_format))
    {
        gl->gbm_surface =
            gbm_surface_create_with_modifiers(process_gbm_device, gl->width, gl->height,
                                              drm_format, dmabuf_format->modifiers.data,
                                              dmabuf_format->modifiers.size / sizeof(uint64_t));
    }
    else
    {
...
static BOOL init_pixel_formats(void)
{
    ...
    for (pass = 0; pass < 2; pass++)
    {
        for (i = 0; i < count; i++)
        {
            ...
            if (!wayland_dmabuf_find_format(dmabuf, visual_id))
                continue;
