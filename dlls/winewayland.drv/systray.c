...
static void create_tooltip(struct tray_icon *icon)
{
    ...
    if (icon->tooltip)
    {
        ...
        NtUserMessageCall( icon->tooltip, TTM_ADDTOOLW, 0, (LPARAM)&ti, NULL,
...
static void update_systray_balloon_position(void)
{
    ...
    NtUserGetWindowRect( balloon_icon->window, &rect );
    pos.x = (rect.left + rect.right) / 2;
    pos.y = (rect.top + rect.bottom) / 2;
    if (pos.x == balloon_pos.x && pos.y == balloon_pos.y) return;  /* nothing changed */
    balloon_pos = pos;
    NtUserMessageCall( balloon_window, TTM_TRACKPOSITION, 0, MAKELONG( pos.x, pos.y ),
...
static void balloon_create_timer( struct tray_icon *icon )
{
    ...
    NtUserMessageCall( balloon_window, TTM_ADDTOOLW, 0, (LPARAM)&ti, NULL, NtUserSendMessage, FALSE );
    if ((icon->info_flags & NIIF_ICONMASK) == NIIF_USER)
        NtUserMessageCall( balloon_window, TTM_SETTITLEW, (WPARAM)icon->info_icon, (LPARAM)icon->info_title,
                           NULL, NtUserSendMessage, FALSE );
    else
        NtUserMessageCall( balloon_window, TTM_SETTITLEW, icon->info_flags, (LPARAM)icon->info_title, NULL,
                           NtUserSendMessage, FALSE );
    balloon_icon = icon;
    balloon_pos.x = balloon_pos.y = MAXLONG;
    update_systray_balloon_position();
    NtUserMessageCall( balloon_window, TTM_TRACKACTIVATE, TRUE, (LPARAM)&ti, NULL, NtUserSendMessage, FALSE );
    NtUserKillTimer( icon->window, BALLOON_CREATE_TIMER );
    NtUserSetTimer( icon->window, BALLOON_SHOW_TIMER, icon->info_timeout, NULL, TIMERV_DEFAULT_COALESCING );
}
...
static BOOL show_balloon( struct tray_icon *icon )
{
    ...
    balloon_icon = icon;
    NtUserSetTimer( icon->window, BALLOON_CREATE_TIMER, BALLOON_CREATE_TIMEOUT, NULL, TIMERV_DEFAULT_COALESCING );
    return TRUE;
}
...
static void hide_balloon(void)
{
    ...
    if (balloon_window)
    {
        NtUserKillTimer( balloon_icon->window, BALLOON_SHOW_TIMER );
        NtUserDestroyWindow( balloon_window );
        balloon_window = 0;
    }
    else NtUserKillTimer( balloon_icon->window, BALLOON_CREATE_TIMER );
    balloon_icon = NULL;
}
...
static void update_tooltip_text(struct tray_icon *icon)
{
    ...
    NtUserMessageCall( icon->tooltip, TTM_UPDATETIPTEXTW, 0, (LPARAM)&ti, NULL,
                       NtUserSendMessage, FALSE );
}
...
static void add_to_standalone_tray( struct tray_icon *icon )
{
    ...
    nb_displayed++;
    size = get_window_size();
    NtUserSetWindowPos( standalone_tray, 0, 0, 0, size.cx, size.cy, SWP_NOMOVE | SWP_NOACTIVATE | SWP_NOZORDER );
    if (nb_displayed == 1 && option_show_systray) NtUserShowWindow( standalone_tray, SW_SHOWNA );
...
static void remove_from_standalone_tray( struct tray_icon *icon )
{
    ...
    LIST_FOR_EACH_ENTRY( ptr, &icon_list, struct tray_icon, entry )
    {
        ...
        NtUserSetWindowPos( ptr->window, 0, pos.x, pos.y, 0, 0, SWP_NOSIZE | SWP_NOACTIVATE | SWP_NOZORDER );
    }
    icon->display = -1;
    if (!--nb_displayed) NtUserShowWindow( standalone_tray, SW_HIDE );
...
static void repaint_tray_icon( struct tray_icon *icon )
{
    ...
    NtUserGetWindowRect( icon->window, &rc );
    ...
    if (!(dib = NtGdiCreateDIBSection( 0, NULL, 0, info, DIB_RGB_COLORS, 0, 0, 0, &color_bits ))) goto done;
    NtGdiSelectBitmap( hdc, dib );
    NtUserDrawIconEx( hdc, pos.x, pos.y, icon->image, width, height, 0, 0, DI_DEFAULTSIZE | DI_NORMAL );
    ...
    if (!has_alpha)
    {
        ...
        if (!(mask = NtGdiCreateDIBSection( 0, NULL, 0, info, DIB_RGB_COLORS, 0, 0, 0, &mask_bits ))) goto done;
        memset( mask_bits, 0xff, width_bytes * size.cy );
        NtGdiSelectBitmap( hdc, mask );
        NtUserDrawIconEx( hdc, pos.x, pos.y, icon->image, width, height, 0, 0, DI_DEFAULTSIZE | DI_MASK );
        ...
        NtGdiSelectBitmap( hdc, dib );
        NtGdiDeleteObjectApp( mask );
    }

    NtUserUpdateLayeredWindow( icon->window, 0, NULL, NULL, hdc, NULL, 0, &blend, ULW_ALPHA, NULL );
done:
...
static BOOL notify_owner( struct tray_icon *icon, UINT msg, LPARAM lparam )
{
    ...
    if (icon->version >= NOTIFYICON_VERSION_4)
    {
        ...
        NtUserClientToScreen( icon->window, &pt );
...
static LRESULT WINAPI tray_icon_wndproc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    if (msg == WM_NCCREATE)
        NtUserSetWindowLongPtr(hwnd, GWLP_USERDATA, (LPARAM)((const CREATESTRUCTW *)lparam)->lpCreateParams, FALSE);

    icon = (struct tray_icon *) NtUserGetWindowLongPtrW(hwnd, GWLP_USERDATA);
    ...
    case WM_PAINT:
        if (!icon->layered)
        {
            ...
            NtUserGetClientRect(hwnd, &rc);
            TRACE("painting rect %s\n", wine_dbgstr_rect(&rc));
            NtUserDrawIconEx( hdc, (rc.left + rc.right - cx) / 2, (rc.top + rc.bottom - cy) / 2,
...
static BOOL hide_icon( struct tray_icon *icon )
{
    ...
    NtUserDestroyWindow(icon->window);
    NtUserDestroyWindow(icon->tooltip);
    icon->window = 0;
    icon->layered = FALSE;
    icon->tooltip = 0;
