...
static const char *idc_hand[] = {"openhand", "fleur",
...
static struct wl_cursor *_wl_cursor_from_wine_cursor(struct wl_cursor_theme *cursor_theme,
                                                     unsigned long int wine_cursor_enum)
{
    ...
    for (i = 0; i < count; i++)
    {
        cursor = wl_cursor_theme_get_cursor(cursor_theme, cursors[i]);
        if (cursor)
            return cursor;
    }
