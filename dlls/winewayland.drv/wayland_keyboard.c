                                  ...
                                  struct wl_array *keys)
{
        ...
        wayland_surface->wayland == wayland)
    {
        ...
        if (!foreground || foreground_is_mapped)
        {
            ...
            NtUserSetForegroundWindow(toplevel->hwnd, FALSE);
        }
    }
}
                        ...
                        LPWSTR buf, int nchars, UINT flags, HKL hkl)
{
    ...
    ret = MultiByteToWideChar(CP_UTF8, 0, utf8, utf8_len, buf, nchars);
...
INT WAYLAND_GetKeyNameText(LONG lparam, LPWSTR buffer, INT size)
{
    ...
    if (!len)
    {
        ...
        len = snprintfW(buffer, size, format, vkey);
        /* Manually null-terminate the string if snprintfW ran out of space. */
        if (len == size) buffer[--len] = 0;
    }
