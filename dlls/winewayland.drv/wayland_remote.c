...
static void wayland_remote_buffer_destroy(struct wayland_remote_buffer *remote_buffer)
{
    ...
    if (remote_buffer->released_event)
    {
        ...
        SetEvent(remote_buffer->released_event);
        CloseHandle(remote_buffer->released_event);
    }
    else
    {
...
static void wayland_remote_throttle_destroy(struct wayland_remote_throttle *remote_throttle)
{
    ...
    {
        SetEvent(remote_throttle->event);
        CloseHandle(remote_throttle->event);
    }
    ...
    if (remote->wayland_surface->drawing_allowed && wl_surface)
    {
        ...
        if (throttle_event &&
            !wayland_remote_throttle_create(remote, wl_surface_frame(wl_surface),
                                            throttle_event))
        {
            SetEvent(throttle_event);
            CloseHandle(throttle_event);
        }
...
err:
    ...
    {
        SetEvent(params->released_event);
        CloseHandle(params->released_event);
    }
    if (params->throttle_event)
    {
        SetEvent(params->throttle_event);
        CloseHandle(params->throttle_event);
    }
...
out:
    ...
    if (params_handle) CloseHandle(params_handle);
}
...
static HANDLE remote_handle_from_local(HANDLE local_handle, HWND remote_hwnd)
{
    ...
    if (!GetWindowThreadProcessId(remote_hwnd, &remote_process_id)) return 0;

    remote_process = OpenProcess(PROCESS_DUP_HANDLE, FALSE, remote_process_id);
    if (!remote_process)
    {
    ...
    if (!DuplicateHandle(GetCurrentProcess(), local_handle, remote_process,
...
out:
    if (local_fd_handle) CloseHandle(local_fd_handle);
                                                                         ...
                                                                         enum wayland_remote_surface_type type)
{
    ...
    PostMessageW(proxy->hwnd, WM_WAYLAND_REMOTE_SURFACE,
                 WAYLAND_REMOTE_SURFACE_MESSAGE_CREATE,
                                         ...
                                         HANDLE *throttle_event_out)
{
    ...
    {
        local_released_event = CreateEventW(NULL, TRUE, FALSE, NULL);
        if (!local_released_event) goto err;
    ...
    {
        local_throttle_event = CreateEventW(NULL, TRUE, FALSE, NULL);
        if (!local_throttle_event) goto err;
...
err:
    ...
    {
        for (i = 0; i < native->plane_count; i++)
            if (params->fds[i]) CloseHandle(params->fds[i]);
        if (params->released_event) CloseHandle(params->released_event);
        munmap(params, sizeof(*params));
    }
    if (params_fd >= 0) close(params_fd);
    if (local_released_event) CloseHandle(local_released_event);
    if (local_throttle_event) CloseHandle(local_throttle_event);
...
BOOL wayland_remote_surface_proxy_dispatch_events(struct wayland_remote_surface_proxy *proxy)
{
    ...
    ret = PostMessageW(proxy->hwnd, WM_WAYLAND_REMOTE_SURFACE,
                       WAYLAND_REMOTE_SURFACE_MESSAGE_DISPATCH_EVENTS,
                       HandleToLong(remote_params_handle));
