...
static RGNDATA *get_region_data(HRGN region)
{
    ...
    if (!(data = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, size))) goto err;
                                   ...
                                   HRGN surface_damage_region)
{
    ...
    if (surface_damage)
    {
        ...
        HeapFree(GetProcessHeap(), 0, surface_damage);
    }
                                            ...
                                            struct wayland_output *output)
{
    ...
    if (surface->main_output != output)
    {
        ...
        if (surface->hwnd)
        {
            NtUserMessageCall(surface->hwnd, WM_WAYLAND_SURFACE_OUTPUT_CHANGE,
                              0, 0, NULL, NtUserSendMessage, FALSE);
        }
    }
}
...
void wayland_surface_set_title(struct wayland_surface *surface, LPCWSTR text)
{
    ...
    if (utf8_count && (utf8 = HeapAlloc(GetProcessHeap(), 0, utf8_count)) &&
