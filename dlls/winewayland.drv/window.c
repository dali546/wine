...
static void wayland_win_data_update_wayland_surface(struct wayland_win_data *data)
{
    ...
    /* Use wayland subsurfaces for windows that we consider to be popups and
     * have an effective parent. Otherwise, if the window is visible make it
     * wayland toplevel. Finally, if the window is not visible create a plain
     * (without a role) surface to avoid polluting the compositor with empty
     * xdg_toplevels. */
    if (parent_surface && wayland_win_data_can_be_popup(data))
