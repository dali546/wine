...
void CDECL wayland_window_surface_flush(struct window_surface *window_surface)
{
    ...
    HeapFree(GetProcessHeap(), 0, buffer_damage);
...
static void CDECL wayland_window_surface_destroy(struct window_surface *window_surface)
{
    ...
    if (wws->wayland_buffer_queue)
        wayland_buffer_queue_destroy(wws->wayland_buffer_queue);
    HeapFree(GetProcessHeap(), 0, wws->bits);
    free(wws->front_bits);
    HeapFree(GetProcessHeap(), 0, wws);
}
                                                     ...
                                                     BOOL src_alpha)
{
    ...
    wws = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(*wws));
    if (!wws) return NULL;
    ...
    if (!(wws->bits = HeapAlloc(GetProcessHeap(), 0, wws->info.bmiHeader.biSizeImage)))
        goto failed;
